...
public class SessionService<TSessionData> where TSessionData : class, new()
{
    ...
    public virtual TSessionData CreateData()
    {
        var data = new TSessionData();

        _dataReference.Value = Guid.NewGuid();

        var dataWrapper = new SessionDataWrapper<TSessionData>(data);
